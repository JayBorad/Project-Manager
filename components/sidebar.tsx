    ...
    <aside className="border-border/80 bg-card/80 flex h-full w-64 shrink-0 flex-col border-r backdrop-blur-sm">
      ...
      <div className="flex h-14 items-center gap-2 px-3">
        <DropdownMenu>
          <DropdownMenuTrigger>
            <span
              className="hover:bg-muted/80 flex flex-1 items-center gap-2 rounded-md px-2 py-2 text-left outline-none transition-colors cursor-pointer"
            >
              ...
              <HugeiconsIcon icon={ArrowDown01Icon} className="h-4 w-4 shrink-0 text-muted-foreground" />
            </span>
          </DropdownMenuTrigger>
        ...
        >
          <HugeiconsIcon icon={theme === "light" ? MoonIcon : SunIcon} className="h-4 w-4" />
        </Button>
      </div>
      ...
      {createWorkspaceOpen && (
        <div className="border-border/80 flex gap-2 border-t px-3 py-2">
          ...
          />
          <Button size="sm" className="h-8 text-xs" onClick={handleCreateWorkspace}>
            Create
          </Button>
          <Button
            variant="ghost"
      ...
      <div className="px-2 py-1">
        ...
        >
          <HugeiconsIcon icon={LinkSquareIcon} className="h-4 w-4 text-muted-foreground" />
          My tasks
        </Link>
      </div>
      ...
      <div className="px-2 py-1">
        <Link
          ...
          className={cn(
            "hover:bg-muted/80 flex items-center gap-2 rounded-md px-2 py-2 text-sm transition-colors",
            pathname === "/dashboard/pull-requests" ? "bg-muted font-medium" : ""
          )}
        >
          <HugeiconsIcon icon={LinkSquareIcon} className="h-4 w-4 text-muted-foreground" />
          Pull requests
      ...
      <div className="mt-2 px-3">
        ...
        <div className="space-y-0.5">
          ...
          <button type="button" className="hover:bg-muted/80 flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm">
        ...
        <div className="space-y-0.5">
          {projects.slice(0, 4).map((project) => (
            ...
            >
              ...
              <HugeiconsIcon icon={ArrowRight01Icon} className="h-3.5 w-3.5 text-muted-foreground" />
            </Link>
          ))}
        </div>
      </div>
